...
(function () {
  ...
  function save() {
    try {
      ...
      toast("Erro ao salvar. O armazenamento pode estar cheio.", "err");
    }
  }
  ...
  function setupLock() {
    ...
    function submit() {
      ...
      if (!hasPassword()) {
        localStorage.setItem(AUTH_KEY, hash(v));
        unlock();
      } else if (localStorage.getItem(AUTH_KEY) === hash(v)) {
        unlock();
      } else {
  ...
  function trocarSenha() {
    ...
    openModal("Trocar senha", body, function (b) {
      b.querySelector("#formSenha").addEventListener("submit", function (e) {
        ...
        localStorage.setItem(AUTH_KEY, hash(s1));
        closeModal();
  ...
  function render() {
    document.getElementById("content").innerHTML = (VIEWS[currentView] || viewDashboard)();
